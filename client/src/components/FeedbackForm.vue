<template>
    <div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center" @click.self="closeForm">
      <div class="feedback-form-container relative bg-white p-10 rounded-lg">
        <!-- Close button -->
        <button class="absolute top-2 right-2 text-black" @click="closeForm">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
          </svg>
        </button>
        <h2 class="text-lg font-bold mb-4">Leave Your Feedback</h2>
        <form @submit.prevent="submitFeedback">
          ...
          <div class="mb-4">
            ...
            <div class="flex space-x-2">
              <label v-for="number in 5" :key="number" class="rating-label">
                ...
                />
                <span class="rating-number" :class="{ 'selected': feedbackRating === number }">{{ number }}</span>
              </label>
            </div>
          </div>
          ...
          <div class="mb-4">
            ...
            <textarea id="feedback" v-model="feedbackText" rows="4" class="w-full p-2 border rounded" placeholder="Tell us what you think..."></textarea>
          </div>
