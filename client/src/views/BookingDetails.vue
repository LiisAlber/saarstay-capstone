  ...
  <div class="flex min-h-screen items-center justify-center bg-[#f7ebe9] p-4">
    ...
    <div class="mx-auto mt-[100px] w-full max-w-lg rounded-lg bg-white p-6 shadow-md">
      ...
      <div v-if="bookingDetails" class="space-y-4">
        <p>
          <strong>{{ t('bookingConfirmation.guestName') }}:</strong> {{ bookingDetails.guestName }}
        </p>
        <p>
          <strong>{{ t('bookingConfirmation.email') }}:</strong> {{ bookingDetails.guestEmail }}
        </p>
        <p>
          <strong>{{ t('bookingConfirmation.contactNumber') }}:</strong>
          {{ bookingDetails.guestContactNumber }}
        </p>
        <p>
          <strong>{{ t('bookingConfirmation.checkInDate') }}:</strong>
          {{ bookingDetails.checkInDate.toLocaleDateString() }}
        </p>
        <p>
          <strong>{{ t('bookingConfirmation.checkOutDate') }}:</strong>
          {{ bookingDetails.checkOutDate.toLocaleDateString() }}
        </p>
        <p>
          <strong>{{ t('bookingConfirmation.numberOfGuests') }}:</strong>
          {{ bookingDetails.numberOfGuests }}
        </p>
        <p v-if="bookingDetails.specialRequests">
          <strong>{{ t('bookingConfirmation.specialRequests') }}:</strong>
          {{ bookingDetails.specialRequests }}
        </p>
        <p>
          <strong>{{ t('bookingConfirmation.totalPrice') }}:</strong>
        ...
        <p class="mt-4 font-medium">
          ...
          {{ t('bookingConfirmation.checkYourInbox') }}
        </p>
