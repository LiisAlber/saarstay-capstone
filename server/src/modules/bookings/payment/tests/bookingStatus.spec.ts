...
it('should update booking status for a valid booking ID', async () => {
    ...
    await testDb.getRepository(Booking).update({ hashedId: fakeBookingInstance.hashedId }, {
    ...
    const updatedBooking = await testDb.getRepository(Booking).findOne({ where: { hashedId: fakeBookingInstance.hashedId } });
