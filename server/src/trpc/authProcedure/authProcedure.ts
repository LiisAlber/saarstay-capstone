...
export type VerifyToken = (token: string) => Jwt | JwtPayload | string
...
export function buildAuthenticatedProcedure(verify: VerifyToken) {
  function getUserFromToken(token: string) {
    try {
      ...
      // console.log('Token Verified:', tokenVerified); // For debugging
      const tokenParsed = tokenSchema.parse(tokenVerified)
      // console.log('Token Parsed:', tokenParsed); // For debugging
      return tokenParsed.user
    } catch (error) {
      // console.error('Error in getUserFromToken:', error); // For debugging
      return null
    }
  }
